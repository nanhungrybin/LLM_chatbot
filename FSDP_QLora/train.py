...
class InstructionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        elif self.style == "ours2":
            prompt_template1 = "Answer the following Question in Korean based on the Context only. Only long and detailed answer from the Context.###Question:\n{Q1}\n###Context:\n{corpus}\n###Answer:\n"
            sample = self.dataset[index]
            prompt1 = prompt_template1.format_map(sample)
            a1 = sample['A1'] if sample['A1'] is not None else ""
            a2 = sample['A2'] if sample['A2'] is not None else ""
            a3 = sample['A3'] if sample['A3'] is not None else ""
            example1 = prompt1 + a1
            
            example=example1
            prompt=prompt1
            #if sample['Q2'] is not None:
            #    prompt_template2="Answer the following Question based on the Context only. Only long and detailed answer from the Context. ####Question:\n{Q2}\n###Context:\n{corpus}\n###Answer:\n"
            #    prompt2=prompt_template2.format_map(sample)
            #    example2=prompt2+a2
            #    example+=example2
            #    prompt+=prompt2
            #elif sample['Q3'] is not None:
            #    prompt_template3="Answer the following Question based on the Context only. Only long and detailed answer from the Context. ###Question:\n{Q3}\n###Context:\n{corpus}\n###Answer:\n"
            #    prompt3=prompt_template3.format_map(sample)
            #    example3=prompt3+a3
            #    example+=example3
            #    prompt+=prompt3
                        
        #elif self.style=="ours":
        #    prompt_template="###Context:\n{context}\n###Question:\n{question}\n###Answer:\n"
        #    prompt=prompt_template.format_map(sample)
        #    example=prompt+sample['answer']
        elif self.style=="ours":
            prompt_template1 = "###Question:\n{Q1}\n###Answer:\n{A1}"
            sample = self.dataset[index]
            prompt1 = prompt_template1.format_map(sample)
            a1 = sample['A1'] if sample['A1'] is not None else ""
            a2 = sample['A2'] if sample['A2'] is not None else ""
...
def get_dataloader(tokenizer:PreTrainedTokenizerFast, args:Dict):
    ...
    if args["dataset"] == "alpaca":
        ...
        dataset=load_dataset("json",data_files="/home/azureuser/cloudfiles/code/Users/hm.lim/Gen/CODE/fsdp_qlora/data/train_data_7.json")
        dataset=dataset['train']
    elif args["dataset"]=="ours2":
        dataset=load_dataset("json",data_files="/home/azureuser/cloudfiles/code/Users/hm.lim/Gen/CODE/fsdp_qlora/data/train_data_7.json")
        dataset=dataset['train']
        print(dataset)
    ...
    if args["dataset"] == "guanaco":
        ...
        dataset = InstructionDataset(dataset, tokenizer, style="ours")
    elif args["dataset"] == "ours2":
        dataset = InstructionDataset(dataset, tokenizer, style="ours2")
    elif args["dataset"] == "orca_math":
...
def fsdp_main(local_rank:int, world_size:int, args:Dict):
    ...
    with profiler_context as prof:
        ...
        for epoch in range(args['num_epochs']):
            ...
            print("epoch: ",epoch,"losses:",losses_cpu)

            try:
                print(sum(losses_cpu)/len(losses_cpu))
            except:
                print("0이다")
        ...
        #plt.savefig('./outputs_ft/ix/ours2/test/그래프.jpg')
        ...
        if args["save_model"]:
            ...
            #model.save_pretrained('./outputs/ours2/test1')
            ...
            if args["train_type"] in ["custom_lora", "custom_qlora", "hqq_lora", "hqq_dora", "bnb_dora", "bnb_llama_pro", "hqq_llama_pro"]:
                ...
                for prefix, module in trainable_fsdp_modules:
                    ...
                    with FSDP.state_dict_type(module, StateDictType.FULL_STATE_DICT, save_policy):
                       ...
                       # model.save_pretrained('./outputs/our2/test2')
                        #ours_state_dict={**ours_state_dict,**{f"{prefix}.{k}":v for k,v in lora.lora_state_dict(module).items()}}
                ...
                if rank==0:
                   ...
                   # model.save_pretrained('./outputs/our2/test2')
                    #torch.save(lora.lora_state_dict(),"./outputs/test")
                    #torch.save(ours_state_dict,"./outputs/test_tt")
                ...
                with FSDP.state_dict_type(model, StateDictType.FULL_STATE_DICT, save_policy):
                    ...
                    if rank==0:
                        ...
                        save_file(cpu_state_dict, os.path.join(args["output_dir"], "model_state_dict.safetensors"))
                       # model.save_pretrained("./outputs/our")
                        #torch.save(lora.lora_state_dict(module),"./outputs/test")
                        #torch.save(ours_state_dict,"./outputs/test_tt")
                        print("Done", rank)
...
def main(
    ...
    gradient_accumulation_steps: int = 1, # How many steps to accumulate gradients over (increases effective batch size)
    num_epochs: int = 1, # How many epochs of training to do
    dataset: Param("", choices=["alpaca", "alpaca_sample", "dummy","ours","ours2", "guanaco", "sql", "orca_math"]) = "alpaca_sample", # alpaca, alpaca_sample (for a 128-sample test) or "dummy" for 16 long dummy samples
    dataset_samples: int = 512, # Number of samples in an epoch if using "alpaca_sample" or "dummy" dataset
